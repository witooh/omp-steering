...
const INCLUSION_MODES: Record<string, true> = { always: true, fileMatch: true, manual: true, auto: true };
const FILE_REFERENCE_PATTERN = /#\[\[file:([^\]]+)\]\]/g;
const DEFAULT_REFERENCE_LIMIT = 50 * 1024;

export function parseSteering(source: string, location: SteeringLocation): SteeringFile {
  const { frontmatter, body } = splitFrontmatter(source);
  const rawInclusion = frontmatter.inclusion ?? "always";

  if (typeof rawInclusion !== "string" || INCLUSION_MODES[rawInclusion] !== true) {
...
export async function discoverSteering(options: { homeDir: string; workspaceRoot: string }): Promise<DiscoveryResult> {
  const locations: Array<{ directory: string; prefix: string; scope: SteeringScope }> = [
    { directory: join(options.homeDir, ".kiro", "steering"), prefix: "~/.kiro/steering", scope: "global" },
    { directory: join(options.workspaceRoot, ".kiro", "steering"), prefix: ".kiro/steering", scope: "workspace" },
  ];
...
export function findNamedSteering(files: SteeringFile[], name: string): SteeringFile | undefined {
...
export function matchFileSteering(files: SteeringFile[], targetPath: string, workspaceRoot: string): SteeringFile[] {
  ...
  return files.filter(
    (file) =>
      ...
      file.patterns.some((pattern) => {
        ...
        // Kiro tolerates basename-only patterns such as "*.tsx" for nested files.
        return glob.match(normalizedTarget) || (!pattern.includes("/") && glob.match(basename(normalizedTarget)));
      }),
  );
}
...
async function findMarkdownFiles(directory: string, prefix = ""): Promise<string[]> {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (entry.isDirectory()) files.push(...(await findMarkdownFiles(join(directory, entry.name), relativePath)));
    else if (entry.isFile() && entry.name.endsWith(".md")) files.push(relativePath);
  }
  return files;
}

async function expandReference(reference: string, workspaceRoot: string, maxBytes: number): Promise<string> {
  if (isAbsolute(reference)) return `[Kiro file reference rejected: ${reference} is outside the workspace]`;
  ...
  try {
    ...
    if (!fileStats.isFile()) return `[Kiro file reference unavailable: ${reference} is not a file]`;
    ...
    const message = (error as NodeJS.ErrnoException).code === "ENOENT" ? "not found" : "unreadable";
