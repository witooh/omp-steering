...
async function makeTemporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), "omp-steering-"));
...
describe("discoverSteering", () => {
  it("discovers nested global and workspace markdown files", async () => {
    const root = await makeTemporaryDirectory();
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    await mkdir(join(home, ".kiro/steering/shared"), { recursive: true });
    await mkdir(join(workspace, ".kiro/steering/frontend"), { recursive: true });
    await writeFile(join(home, ".kiro/steering/shared/review.md"), "Global review");
    await writeFile(join(workspace, ".kiro/steering/frontend/review.md"), "Workspace review");
    await writeFile(join(workspace, ".kiro/steering/ignored.txt"), "Ignored");

    const result = await discoverSteering({ homeDir: home, workspaceRoot: workspace });

    expect(result.errors).toEqual([]);
    expect(result.files.map((file) => `${file.scope}:${file.displayPath}`)).toEqual([
  ...
  it("reports invalid steering instead of loading it", async () => {
    const root = await makeTemporaryDirectory();
    const workspace = join(root, "workspace");
    await mkdir(join(workspace, ".kiro/steering"), { recursive: true });
    await writeFile(join(workspace, ".kiro/steering/broken.md"), "---\ninclusion: sometimes\n---\nBody");

    const result = await discoverSteering({ homeDir: join(root, "home"), workspaceRoot: workspace });

    expect(result.files).toEqual([]);
    expect(result.errors[0]).toContain("unsupported inclusion mode: sometimes");
  });
});
...
describe("matchFileSteering", () => {
  it("matches Kiro glob patterns against workspace-relative paths", () => {
    ...
    expect(matchFileSteering([steering], "/workspace/src/Button.tsx", "/workspace")).toEqual([steering]);
    expect(matchFileSteering([steering], "src/Button.tsx", "/workspace")).toEqual([steering]);
    expect(matchFileSteering([steering], "/workspace/src/button.ts", "/workspace")).toEqual([]);
    expect(matchFileSteering([steering], "/outside/Button.tsx", "/workspace")).toEqual([]);
  });

  it("honours directory-scoped patterns", () => {
    ...
    expect(matchFileSteering([steering], "src/api/users/list.ts", "/workspace")).toEqual([steering]);
    expect(matchFileSteering([steering], "src/web/list.ts", "/workspace")).toEqual([]);
  });
});
...
describe("expandFileReferences", () => {
  it("inlines live workspace file references", async () => {
    ...
    const expanded = await expandFileReferences("Follow #[[file:docs/api.md]]", workspace);
  ...
  it("does not read file references outside the workspace", async () => {
    ...
    const expanded = await expandFileReferences("#[[file:../secret.txt]]", workspace);
